...
export const LanguageSelector: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    client
      .fetch<string[]>(
        `*[_type == "languageRight" && _id != $id].languageCode`,
        { id: documentId ?? '' }
      )
      .then((codes) => setExistingCodes(new Set(codes)))
  }, [client, documentId])
  ...
  return (
    <Card>
      {props.value ? (
        <label>{languageCodeNamesMap[props.value] || props.value}</label>
      ) : (
        <Autocomplete
          filterOption={(query, option) => {
            ...
            if (
              ...
              nativeName?.toLowerCase().includes(query.toLowerCase())
            ) {
              return true
            } else {
              return false
          ...
          options={Object.keys(languageCodeNamesMap).map((code) => ({
            value: code,
          }))}
          renderOption={(option) => (
            <label>{languageCodeNamesMap[option.value]}</label>
          )}
