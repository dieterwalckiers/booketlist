...
export function getUniqueAvailableLanguageRights(
  ...
  return languageRights.filter(
    (lang) => languageRightCount[lang.code] !== books.length
  )
}
...
export function filterBooks(books: Book[], bookFilter: BookFilter) {
  ...
  return books.filter((book) => {
    if (bookFilter.searchString) {
      if (
        !book.searchableDataSerialized
          .toLowerCase()
          .includes(bookFilter.searchString.toLowerCase())
      ) {
        return false
      }
    }
    ...
    if (bookFilter.authors && bookFilter.authors.length > 0) {
      if (
        !bookFilter.authors.some((a) => book.authors.some((b) => b.slug === a))
      ) {
        return false
      }
    }
    if (bookFilter.illustrators && bookFilter.illustrators.length > 0) {
      if (
        !bookFilter.illustrators.some((a) =>
          book.illustrators.some((b) => b.slug === a)
        )
      ) {
        return false
      }
    ...
    if (bookFilter.avLangRights && bookFilter.avLangRights.length > 0) {
      if (
        bookFilter.avLangRights.some((a) =>
          book.soldLanguageRights.some((b) => b.code === a)
        )
      ) {
        return false
      }
