...
import {
  getUniqueAuthors,
  getUniqueAvailableLanguageRights,
  getUniqueCategories,
  getUniqueIllustrators,
  getUniquePublishers,
} from './helpers'

const FilterAutocompleteWithNoSSR = dynamic(import('./filterautocomplete'), {
...
function checkablesFromBookCategories(
...
function acItemsFromAvailableLanguageRights(
  availableLanguageRights: LanguageRight[]
): AcItem[] {
...
const FilterPanel: React.FC<Props> = ({
  ...
  const availableLanguageRights = useMemo(
    () => getUniqueAvailableLanguageRights(languageRights, books),
    [languageRights, books]
  )

  const onUpdateBookCategories = useCallback(
    (slugs: string[]) => {
      onUpdateFilter({
        ...bookFilter,
        bookCats: slugs,
  ...
  const onUpdateFromToAge = useCallback(
  ...
  const onUpdateAuthors = useCallback(
    (slugs: string[]) => {
      onUpdateFilter({
        ...bookFilter,
        authors: slugs,
  ...
  const onUpdateIllustrators = useCallback(
    (slugs: string[]) => {
      onUpdateFilter({
        ...bookFilter,
        illustrators: slugs,
  ...
  const onUpdatePublishers = useCallback(
    (slugs: string[]) => {
      onUpdateFilter({
        ...bookFilter,
        publishers: slugs,
  ...
  const onUpdateAvLanguageRights = useCallback(
    (avLangRights: string[]) => {
      onUpdateFilter({
        ...bookFilter,
        avLangRights,
  ...
  return filterAvailable ? (
    <Box
      w={{ base: 'calc(100vw - 2rem)', md: '280px' }}
      className="filterpanel"
      flexShrink={0}
    >
      <Flex alignItems="center">
        <Heading as="h2" color="#444" fontSize="xl">
          Filter
        </Heading>
      ...
      <FilterPanelSection title="Search">
        <FilterFreeTextSearch
          ...
          onUpdateSearchString={(searchString) =>
            onUpdateFilter({ ...bookFilter, searchString })
          }
        />
      </FilterPanelSection>
